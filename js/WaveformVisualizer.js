...
console.log('[WaveformVisualizer] Module loaded');
    ...
    ctx.fillStyle = colorWithAlpha(baseColor, 0.85);
    const drawY = 18;
    const drawH = height - drawY;

    if (channelMode === 'split' && numChannels > 1) {
        const half = Math.floor(drawH / 2) - 1;
        drawChannelBlock(ctx, audioBuffer.getChannelData(0), drawY, width, half, samplesPerCol, mode);
        drawChannelBlock(ctx, audioBuffer.getChannelData(1), drawY + half + 2, width, half, samplesPerCol, mode);
        ctx.fillStyle = 'rgba(148,163,184,0.6)';
        ctx.font = '10px sans-serif';
        ctx.textBaseline = 'top';
        ctx.fillText('L', 4, drawY + 4);
        ctx.fillText('R', 4, drawY + half + 6);
    } else {
        const left = audioBuffer.getChannelData(0);
        const right = numChannels > 1 ? audioBuffer.getChannelData(1) : null;
        for (let x = 0; x < width; x++) {
    ...
    const startSec = startSample / Math.max(1, sampleRate);
    const endSec = endSample / Math.max(1, sampleRate);
    ctx.fillStyle = 'rgba(0,0,0,0.55)';
    ctx.fillRect(width - 140, 2, 138, 14);
    ctx.fillStyle = '#e2e8f0';
    ctx.font = '11px sans-serif';
    ctx.textBaseline = 'middle';
    ctx.fillText(startSec.toFixed(2) + 's - ' + endSec.toFixed(2) + 's', width - 134, 9);
}

function renderThumbnails(container) {
    const items = listAudioClips();
    if (items.length === 0) {
        container.innerHTML = '<div class="text-xs text-gray-400 px-2 py-3">No audio clips found. Add an audio track and drop an audio clip onto it.</div>';
        return;
    }
    let html = '';
    for (const item of items) {
        const color = settings.colorByTrack ? (item.track.color || '#60a5fa') : '#60a5fa';
        const startSec = item.startTime.toFixed(2);
        const durSec = item.duration.toFixed(2);
        html += '' +
            '<div class="wv-thumb-card bg-gray-900 border border-gray-800 rounded p-2 mb-2" data-clip-id="' + escapeHtml(item.clipId) + '">' +
                '<div class="flex items-center justify-between mb-1">' +
                    '<div class="text-sm text-gray-200 truncate flex-1" title="' + escapeHtml(item.clipName) + '">' + escapeHtml(item.clipName) + '</div>' +
                    '<div class="text-xs text-gray-500 whitespace-nowrap ml-2">' + escapeHtml(item.trackName) + '</div>' +
                '</div>' +
                '<div class="text-xs text-gray-500 mb-1">start ' + startSec + 's | duration ' + durSec + 's</div>' +
                '<canvas class="wv-thumb" data-clip-id="' + escapeHtml(item.clipId) + '" data-track-color="' + escapeHtml(color) + '" width="' + MAX_THUMBNAIL_WIDTH + '" height="' + MAX_THUMBNAIL_HEIGHT + '" style="width:100%;max-width:' + MAX_THUMBNAIL_WIDTH + 'px;height:' + MAX_THUMBNAIL_HEIGHT + 'px;display:block;background:#0f172a;border-radius:4px;"></canvas>' +
                '<div class="flex gap-2 mt-1">' +
                    '<button class="wv-view-btn px-2 py-1 rounded bg-blue-700 hover:bg-blue-600 text-xs text-white" data-clip-id="' + escapeHtml(item.clipId) + '">View</button>' +
                '</div>' +
            '</div>';
    }
    ...
    const thumbs = container.querySelectorAll('canvas.wv-thumb');
    thumbs.forEach(canvas => {
        const clipId = canvas.getAttribute('data-clip-id');
        const color = canvas.getAttribute('data-track-color') || '#60a5fa';
        ...
        try {
            ...
            console.warn('[WaveformVisualizer] Thumbnail draw failed for clip', clipId, e);
        }
    });

    // Wire view buttons
    container.querySelectorAll('.wv-view-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const cid = btn.getAttribute('data-clip-id');
            settings.selectedClipId = cid;
            settings.zoom = 1;
            settings.pan = 0;
            rerenderPanel();
        });
...
function renderSelectedView(container) {
    if (!settings.selectedClipId) {
        container.innerHTML = '<div class="text-xs text-gray-400 px-2 py-3">Select a clip above (View) to see a zoomable waveform.</div>';
        return;
    }
    const clip = getClipById(settings.selectedClipId);
    if (!clip) {
        container.innerHTML = '<div class="text-xs text-gray-400 px-2 py-3">Selected clip not found.</div>';
        return;
    }
    const color = settings.colorByTrack ? (clip.track.color || '#60a5fa') : '#60a5fa';
    const html = '' +
        '<div class="bg-gray-900 border border-gray-800 rounded p-2 mb-2">' +
            '<div class="flex items-center justify-between mb-1">' +
                '<div class="text-sm text-gray-200 truncate flex-1">' + escapeHtml(clip.clipName) + '</div>' +
                '<div class="text-xs text-gray-500 whitespace-nowrap ml-2">' + escapeHtml(clip.trackName) + '</div>' +
            '</div>' +
            '<div class="text-xs text-gray-500 mb-1">duration ' + clip.duration.toFixed(2) + 's | ' + (clip.buffer.sampleRate || 44100) + ' Hz | ' + Math.max(1, clip.buffer.numberOfChannels || 1) + ' ch</div>' +
            '<canvas id="wv-full-canvas" width="800" height="' + FULL_WAVEFORM_HEIGHT + '" style="width:100%;height:' + FULL_WAVEFORM_HEIGHT + 'px;display:block;background:#0f172a;border-radius:4px;"></canvas>' +
            '<div class="flex items-center gap-2 mt-2 flex-wrap">' +
                '<label class="text-xs text-gray-400">Zoom ' + settings.zoom.toFixed(1) + 'x</label>' +
                '<input id="wv-zoom" type="range" min="1" max="20" step="0.1" value="' + settings.zoom + '" class="flex-1">' +
                '<label class="text-xs text-gray-400">Pan ' + Math.round(settings.pan * 100) + '%</label>' +
                '<input id="wv-pan" type="range" min="0" max="100" step="1" value="' + Math.round(settings.pan * 100) + '" class="flex-1">' +
                '<button id="wv-reset-view" class="px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-xs text-white">Reset</button>' +
                '<button id="wv-export-png" class="px-2 py-1 rounded bg-emerald-700 hover:bg-emerald-600 text-xs text-white">Export PNG</button>' +
            '</div>' +
        '</div>';
    container.innerHTML = html;

    const canvas = container.querySelector('#wv-full-canvas');
    drawFullWaveform(canvas, clip.buffer, {
        mode: settings.mode,
        channelMode: settings.channelMode,
        color: color,
        zoom: settings.zoom,
    ...
    container.querySelector('#wv-zoom').addEventListener('input', (e) => {
        settings.zoom = parseFloat(e.target.value) || 1;
        // Keep the visible window centered around current pan when zoom changes
        const visibleFraction = 1 / Math.max(0.01, settings.zoom);
        if (settings.pan + visibleFraction > 1) settings.pan = Math.max(0, 1 - visibleFraction);
        renderSelectedView(container);
    });
    container.querySelector('#wv-pan').addEventListener('input', (e) => {
        const v = parseFloat(e.target.value) || 0;
        const visibleFraction = 1 / Math.max(0.01, settings.zoom);
        settings.pan = Math.max(0, Math.min(1 - visibleFraction, v / 100));
        renderSelectedView(container);
    });
    container.querySelector('#wv-reset-view').addEventListener('click', () => {
        settings.zoom = 1;
        settings.pan = 0;
        renderSelectedView(container);
    });
    container.querySelector('#wv-export-png').addEventListener('click', () => {
        try {
            const dataUrl = canvas.toDataURL('image/png');
            const a = document.createElement('a');
            a.href = dataUrl;
            a.download = 'waveform-' + (clip.clipName || clip.clipId) + '.png';
            document.body.appendChild(a);
            a.click();
            a.remove();
            notify('Exported waveform PNG', 1500);
        } catch (e) {
            console.warn('[WaveformVisualizer] Export failed:', e);
            notify('Export failed: ' + e.message, 2500);
        }
    });
}
...
function renderSettings(container) {
    container.innerHTML = '' +
        '<div class="flex items-center gap-2 flex-wrap text-xs text-gray-300">' +
            '<label class="flex items-center gap-1">' +
                '<input type="radio" name="wv-mode" value="peak" ' + (settings.mode === 'peak' ? 'checked' : '') + '> Peak' +
            '</label>' +
            '<label class="flex items-center gap-1">' +
                '<input type="radio" name="wv-mode" value="rms" ' + (settings.mode === 'rms' ? 'checked' : '') + '> RMS' +
            '</label>' +
            '<span class="text-gray-600">|</span>' +
            '<label class="flex items-center gap-1">' +
                '<input type="radio" name="wv-channel" value="mono" ' + (settings.channelMode === 'mono' ? 'checked' : '') + '> Mono mix' +
            '</label>' +
            '<label class="flex items-center gap-1">' +
                '<input type="radio" name="wv-channel" value="split" ' + (settings.channelMode === 'split' ? 'checked' : '') + '> Split L/R' +
            '</label>' +
            '<span class="text-gray-600">|</span>' +
            '<label class="flex items-center gap-1">' +
                '<input type="checkbox" id="wv-color-by-track" ' + (settings.colorByTrack ? 'checked' : '') + '> Color by track' +
            '</label>' +
            '<button id="wv-refresh" class="px-2 py-1 rounded bg-gray-700 hover:bg-gray-600 text-white">Refresh</button>' +
            '<button id="wv-close" class="px-2 py-1 rounded bg-red-700 hover:bg-red-600 text-white">Close</button>' +
        '</div>';
    container.querySelectorAll('input[name="wv-mode"]').forEach(el => {
        el.addEventListener('change', (e) => {
            settings.mode = e.target.value;
            rerenderPanel();
        });
    });
    container.querySelectorAll('input[name="wv-channel"]').forEach(el => {
        el.addEventListener('change', (e) => {
            settings.channelMode = e.target.value;
            rerenderPanel();
        });
    });
    container.querySelector('#wv-color-by-track').addEventListener('change', (e) => {
        settings.colorByTrack = !!e.target.checked;
        rerenderPanel();
    });
    container.querySelector('#wv-refresh').addEventListener('click', () => rerenderPanel());
    container.querySelector('#wv-close').addEventListener('click', () => closePanel());
}

function rerenderPanel() {
    const root = document.getElementById(PANEL_CONTENT_ID);
    if (!root) return;
    const settingsRow = root.querySelector('#wv-settings-row');
    const thumbsRow = root.querySelector('#wv-thumbs-row');
    const detailRow = root.querySelector('#wv-detail-row');
    if (settingsRow) renderSettings(settingsRow);
    if (thumbsRow) renderThumbnails(thumbsRow);
    if (detailRow) renderSelectedView(detailRow);
}

function closePanel() {
    isPanelOpen = false;
    settings.selectedClipId = null;
    settings.zoom = 1;
    settings.pan = 0;
    if (localAppServices.getOpenWindows) {
        const wins = localAppServices.getOpenWindows();
        const existing = wins && wins.get && wins.get(WINDOW_ID);
...
export function openWaveformVisualizerPanel() {
    ...
    const container = document.createElement('div');
    container.id = PANEL_CONTENT_ID;
    container.className = 'p-3 bg-gray-950 text-white h-full flex flex-col gap-3 overflow-y-auto';

    const settingsRow = document.createElement('div');
    settingsRow.id = 'wv-settings-row';
    container.appendChild(settingsRow);

    const thumbsRow = document.createElement('div');
    thumbsRow.id = 'wv-thumbs-row';
    container.appendChild(thumbsRow);

    const detailRow = document.createElement('div');
    detailRow.id = 'wv-detail-row';
...
if (typeof window !== 'undefined') {
    ...
    window.isWaveformVisualizerActive = isWaveformVisualizerActive;
    window.openWaveformVisualizerPanel = openWaveformVisualizerPanel;
    window.waveformVisualizerListAudioClips = listAudioClips;
    window.waveformVisualizerGetSettings = () => Object.assign({}, settings);
}
